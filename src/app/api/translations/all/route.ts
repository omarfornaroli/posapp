...
export async function GET() {
  ...
  try {
    ...
    if (enabledLanguages.length > 0) {
      ...
      console.warn('[API/translations/all] No enabled languages found in AppLanguage collection. Falling back to default locales [en, es].');
      activeLocales = ['en', 'es'];
    }
    ...
    const translationsData = translationsFromDb.map(t => {
      ...
      } else if (t.values instanceof Map) {
    ...
    console.error("Error in /api/translations/all:", error);
