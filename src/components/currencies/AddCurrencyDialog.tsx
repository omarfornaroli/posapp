...
interface AddCurrencyDialogProps {
  ...
  onAddCurrency: (newCurrencyData: Omit<Currency, 'id'>) => void;
  existingCodes: string[];
}

export default function AddCurrencyDialog({ open, onOpenChange, onAddCurrency, existingCodes }: AddCurrencyDialogProps) {
  ...
  const currentSchema = currencyFormSchema(t, existingCodes);

  const form = useForm<CurrencyFormData>({
    resolver: zodResolver(currentSchema),
    defaultValues: {
      name: '', code: '', symbol: '', decimalPlaces: 2, isEnabled: true, isDefault: false, exchangeRate: ''
    },
  });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg flex flex-col max-h-[90vh]">
        <DialogHeader>
          <DialogTitle className="font-headline">{t('AddCurrencyDialog.title')}</DialogTitle>
          <DialogDescription>{t('AddCurrencyDialog.description')}</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col flex-grow overflow-hidden">
            ...
            <DialogFooter className="pt-4 mt-4 border-t shrink-0">
              <DialogClose asChild><Button type="button" variant="outline">{t('AddCurrencyDialog.cancelButton')}</Button></DialogClose>
              <Button type="submit" className="bg-primary hover:bg-primary/90">{t('AddCurrencyDialog.addButton')}</Button>
            </DialogFooter>
          </form>
        </Form>
      </DialogContent>
