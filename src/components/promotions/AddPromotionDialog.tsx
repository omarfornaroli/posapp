...
interface AddPromotionDialogProps {
  ...
  onAddPromotion: (newPromotionData: Omit<Promotion, 'id'>) => void;
}

export default function AddPromotionDialog({ open, onOpenChange, onAddPromotion }: AddPromotionDialogProps) {
  ...
  useEffect(() => {
    if (!isLoadingTranslations && !isLoadingRelatedData && open) {
      form.trigger();
    }
  }, [isLoadingTranslations, isLoadingRelatedData, open, form, t]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl flex flex-col max-h-[90vh]">
        <DialogHeader>
          <DialogTitle className="font-headline">{t('AddPromotionDialog.title')}</DialogTitle>
          <DialogDescription>{t('AddPromotionDialog.description')}</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col flex-grow overflow-hidden">
            <ScrollArea className="flex-grow pr-6 -mr-6">
                <div className="space-y-4 py-4 pr-6">
                  ...
                  <FormField control={form.control} name="applicableCategoryIds" render={({ field }) => (<FormItem><FormLabel>{t('AddPromotionDialog.applicableCategoriesLabel')}</FormLabel><DropdownMenu><DropdownMenuTrigger asChild><FormControl><Button variant="outline" className="w-full justify-between">{field.value?.length > 0 ? `${field.value.length} ${t(field.value.length === 1 ? 'AddPromotionDialog.categorySelectedSingular' : 'AddPromotionDialog.categorySelectedPlural')}` : t('AddPromotionDialog.selectCategoriesPlaceholder')}<ChevronDown className="ml-2 h-4 w-4 opacity-50" /></Button></FormControl></DropdownMenuTrigger><DropdownMenuContent className="w-[--radix-dropdown-menu-trigger-width] max-h-60 overflow-y-auto"><DropdownMenuLabel>{t('AddPromotionDialog.availableCategoriesHint')}</DropdownMenuLabel><DropdownMenuSeparator />{categories.length === 0 ? (<DropdownMenuItem disabled>{t('AddPromotionDialog.noCategoriesAvailable')}</DropdownMenuItem>) : categories.map((cat) => (<DropdownMenuCheckboxItem key={cat} checked={field.value?.includes(cat)} onCheckedChange={(checked) => field.onChange(checked ? [...(field.value || []), cat] : (field.value || []).filter(id => id !== cat))}>{cat}</DropdownMenuCheckboxItem>))}</DropdownMenuContent></DropdownMenu><FormMessage /></FormItem>)} />
                  <FormField control={form.control} name="applicableProductIds" render={({ field }) => (<FormItem><FormLabel>{t('AddPromotionDialog.applicableProductsLabel')}</FormLabel><DropdownMenu><DropdownMenuTrigger asChild><FormControl><Button variant="outline" className="w-full justify-between">{field.value?.length > 0 ? `${field.value.length} ${t(field.value.length === 1 ? 'AddPromotionDialog.productSelectedSingular' : 'AddPromotionDialog.productSelectedPlural')}` : t('AddPromotionDialog.selectProductsPlaceholder')}<ChevronDown className="ml-2 h-4 w-4 opacity-50" /></Button></FormControl></DropdownMenuTrigger><DropdownMenuContent className="w-[--radix-dropdown-menu-trigger-width] max-h-60 overflow-y-auto"><DropdownMenuLabel>{t('AddPromotionDialog.availableProductsHint')}</DropdownMenuLabel><DropdownMenuSeparator />{products.length === 0 ? (<DropdownMenuItem disabled>{t('AddPromotionDialog.noProductsAvailable')}</DropdownMenuItem>) : products.map((prod) => (<DropdownMenuCheckboxItem key={prod.id} checked={field.value?.includes(prod.id)} onCheckedChange={(checked) => field.onChange(checked ? [...(field.value || []), prod.id] : (field.value || []).filter(id => id !== prod.id))}>{prod.name} ({prod.barcode})</DropdownMenuCheckboxItem>))}</DropdownMenuContent></DropdownMenu><FormMessage /></FormItem>)} />
                  <FormField control={form.control} name="applicableClientIds" render={({ field }) => (<FormItem><FormLabel>{t('AddPromotionDialog.applicableClientsLabel')}</FormLabel><DropdownMenu><DropdownMenuTrigger asChild><FormControl><Button variant="outline" className="w-full justify-between">{field.value?.length > 0 ? `${field.value.length} ${t(field.value.length === 1 ? 'AddPromotionDialog.clientSelectedSingular' : 'AddPromotionDialog.clientSelectedPlural')}` : t('AddPromotionDialog.selectClientsPlaceholder')}<ChevronDown className="ml-2 h-4 w-4 opacity-50" /></Button></FormControl></DropdownMenuTrigger><DropdownMenuContent className="w-[--radix-dropdown-menu-trigger-width] max-h-60 overflow-y-auto"><DropdownMenuLabel>{t('AddPromotionDialog.availableClientsHint')}</DropdownMenuLabel><DropdownMenuSeparator />{clients.length === 0 ? (<DropdownMenuItem disabled>{t('AddPromotionDialog.noClientsAvailable')}</DropdownMenuItem>) : clients.map((client) => (<DropdownMenuCheckboxItem key={client.id} checked={field.value?.includes(client.id)} onCheckedChange={(checked) => field.onChange(checked ? [...(field.value || []), client.id] : (field.value || []).filter(id => id !== client.id))}>{client.name} ({client.email})</DropdownMenuCheckboxItem>))}</DropdownMenuContent></DropdownMenu><FormMessage /></FormItem>)} />
