...
const countryFormSchema = (t: Function, existingCodes: string[]) => z.object({
  ...
  codeAlpha2: z.string().length(2, { message: t('AddCountryDialog.codeAlpha2LengthError') }).regex(/^[A-Z]{2}$/, { message: t('AddCountryDialog.codeAlpha2FormatError')})
    .refine(code => !existingCodes.includes(code.toUpperCase()), { message: t('AddCountryDialog.codeExistsError') }),
...
interface AddCountryDialogProps {
  ...
  onAddCountry: (newCountryData: Omit<Country, 'id'>) => void;
  existingCodes: string[];
}

export default function AddCountryDialog({ open, onOpenChange, onAddCountry, existingCodes }: AddCountryDialogProps) {
  ...
  const currentSchema = countryFormSchema(t, existingCodes);

  const form = useForm<CountryFormData>({
    resolver: zodResolver(currentSchema),
    defaultValues: {
      name: '', codeAlpha2: '', codeAlpha3: '', numericCode: '', currencyCode: '', flagImageUrl: '', isEnabled: true, isDefault: false,
    },
  });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg flex flex-col max-h-[90vh]">
        <DialogHeader>
          <DialogTitle className="font-headline">{t('AddCountryDialog.title')}</DialogTitle>
          <DialogDescription>{t('AddCountryDialog.description')}</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col flex-grow overflow-hidden">
            ...
            <DialogFooter className="pt-4 mt-4 border-t shrink-0">
              <DialogClose asChild><Button type="button" variant="outline">{t('AddCountryDialog.cancelButton')}</Button></DialogClose>
              <Button type="submit" className="bg-primary hover:bg-primary/90">{t('AddCountryDialog.addButton')}</Button>
            </DialogFooter>
          </form>
        </Form>
      </DialogContent>
