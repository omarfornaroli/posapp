...
export default function EditClientDialog({ open, onOpenChange, client, onSaveClient }: EditClientDialogProps) {
  ...
  useEffect(() => {
    if (client && open && !isLoadingTranslations) {
      form.reset({
        name: client.name,
        email: client.email,
        phone: client.phone || '',
  ...
  }, [client, form, open, isLoadingTranslations, t]);
  
  useEffect(() => {
    if (!isLoadingTranslations && client && open) {
      form.trigger();
    }
  }, [isLoadingTranslations, form, client, open, t]);
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => {
      ...
      <DialogContent className="sm:max-w-[525px] flex flex-col max-h-[90vh]">
        <DialogHeader>
          <DialogTitle className="font-headline">{t('EditClientDialog.title')}</DialogTitle>
          <DialogDescription>
            {t('EditClientDialog.description', { clientName: client?.name || 'client' })}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col flex-grow overflow-hidden">
            ...
            <DialogFooter className="pt-4 border-t mt-auto shrink-0">
              ...
              <Button type="submit" className="bg-primary hover:bg-primary/90">{t('EditClientDialog.saveButton')}</Button>
            </DialogFooter>
          </form>
        </Form>
      </DialogContent>
